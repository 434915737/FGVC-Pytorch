...
def get_transform(resize, phase='train'):
    if phase == 'train':
        return transforms.Compose([
            ...
            # MosaicBlur(p=0.5, mosaic_size=8),  # Add MosaicBlur transform
            transforms.ToTensor(),
...
# class MosaicBlur:
#     def __init__(self, p=0.5, mosaic_size=8):
#         self.p = p
...
#             img_np = np.array(img)
#             h, w, c = img_np.shape
#             img_np = img_np.reshape(h // self.mosaic_size, self.mosaic_size, -1, c).mean(axis=1)
#             img_np = img_np.reshape(-1, w // self.mosaic_size, self.mosaic_size, c).mean(axis=2)
#             img_np = img_np.repeat(self.mosaic_size, axis=0).repeat(self.mosaic_size, axis=1)
#             img = Image.fromarray(np.uint8(img_np), 'RGB')  # Specify the mode explicitly as 'RGB'
#         return img
