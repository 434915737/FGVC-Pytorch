...
class WSDAN(nn.Module):
    ...
    def forward(self, x):
        ...
        assert len(feature_maps.shape) == 4, f"Expected 4D feature_maps, but got {len(feature_maps.shape)}D."

        #ad_block
        #feature_maps = self.ad_block(feature_maps)
        #cbam
        #feature_maps = self.cbam(feature_maps)  # 应用CBAM模块
        # Applying the SE-ASPP module
        #feature_maps = self.se_aspp(feature_maps)
        #feature_maps=self.cbam_aspp(feature_maps)
